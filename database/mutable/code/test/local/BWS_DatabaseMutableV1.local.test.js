...
beforeAll(async () => {
  ...
  var contractAddress;
  var error = false;
  await deploy('BWS_DatabaseMutableV1', secrets)
  ...
  if (!error) {
    ...
    let abi = JSON.parse(fs.readFileSync(path.join(__dirname, '../../build/BWS_DatabaseMutableV1.abi')));
...
test("insert string ('row1','hello world, this is a second string more than 32 bytes string')", async () => {
  await contract.methods.insertString(web3.utils.toHex('id'), web3.utils.toHex('row1'), web3.utils.toHex('hello world, this is a second string more than 32 bytes string')).send({ from: account.address }).then(function (result) {
    expect(true).toBe(true);
  });
});
...
test("select string ('row1')", async () => {
  await contract.methods.selectString(web3.utils.toHex('id'), web3.utils.toHex('row1')).call({ from: account.address }).then(function (result) {
    expect(web3.utils.toHex(web3.utils.hexToAscii(result))).toBe(web3.utils.toHex('hello world, this is a second string more than 32 bytes string'));
  });
});